...
public final class SpiritManager implements Listener {
    ...
    private final Map<UUID, Long> lastClaimExp = new HashMap<>();
    private final Map<UUID, Integer> hourlyKills = new HashMap<>();
    private final Map<UUID, Long> lastKillReset = new HashMap<>();
    ...
    public void addSpiritExperience(Clan clan, long amount, String reason) {
        ...
        long nextLevelExp = getExpForNextLevel(currentLevel);
        ...
        clan.setSpirit(clan.spirit().withLevel(newLevel).addEnergy(amount));
